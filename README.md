...
## M8: SmartMoney (SMC) 策略

> 聪明钱概念策略：跨周期结构交易，BOS + OB 入场

### 核心思想

SmartMoney 策略基于机构订单流概念，通过大周期（HTF）识别结构性突破（BOS）和订单块（OB），在小周期（LTF）等待价格回归 OB 区域时入场。

### 策略特点

| 特性 | 说明 |
|------|------|
| **跨周期** | M15 识别结构 + M1 入场确认 |
| **自适应** | 趋势（BOS顺势）+ 震荡（OB反转）都能交易 |
| **Regime 过滤** | **不受限** - SMC 本身适应多种市场状态 |
| **退出机制** | RR 分批平仓 + 保本 + 移动止损 |
...
| 类型 | 设置方式 | 说明 |
|------|----------|------|
| **初始止损** | OB 边缘 ± ATR 缓冲 | 结构性止损 |
| **止盈目标** | 风险 × 3.5 | 高盈亏比 |
| **部分平仓** | RR=1.0 时平 50% | 锁定部分利润 |
| **保本** | RR=1.0 时移动止损到入场价 | 风险归零 |
| **移动止损** | 盈利 > 2ATR 后启动 | 趋势跟随 |

### Zone 失效机制

```
1. 超时失效: Zone 形成后超过 N 根 HTF K 线
2. 深度打穿: 价格穿透 OB 超过 2 倍 ATR
```
...
```mql5
// 周期设置
SMC_HTF = PERIOD_M15       // 大周期
SMC_LTF = PERIOD_M1        // 小周期

// BOS 检测
SMC_LookbackBOS = 20       // 结构回溯 K 线数
SMC_MinBodyATR = 1.2       // 爆发 K 线实体 ATR 倍数

// Zone 管理
SMC_ZoneExpireBars = 48    // Zone 有效时间

// 风险管理
SMC_SL_ATR_Buffer = 0.5    // 止损 ATR 缓冲
SMC_TP_RR = 3.5            // 止盈盈亏比
SMC_PartialPercent = 50.0  // 部分平仓比例
SMC_BE_RR_Level = 1.0      // 保本触发 RR
```
