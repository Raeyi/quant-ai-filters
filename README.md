...
│  │  H1 主判定："现在是什么市场结构"                       │    │
│  │  M15 过滤："现在能不能做"                             │    │
│  │  输出: Regime + Confidence + TradePermission         │    │
│  └─────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────┘
                         ↓
┌─────────────────────────────────────────────────────────────┐
...
│  │  T+V/T+L → TrendPullback                             │    │
│  │  R+L     → BollMeanReversion                         │    │
│  │  R+V     → STANDBY (风控模式)                        │    │
│  └─────────────────────────────────────────────────────┘    │
│  同时只有一个策略 ACTIVE，其他 STANDBY                       │
└─────────────────────────────────────────────────────────────┘
                         ↓
┌─────────────────────────────────────────────────────────────┐
│                  Active Strategy                             │
...
| Regime | 趋势 | 波动率 | 特征 | 激活策略 |
|--------|------|--------|------|----------|
| **T+V** | 强趋势 | 高波动 | 单边突破，快速移动 | TrendPullback |
| **T+L** | 强趋势 | 低波动 | 稳步趋势，回调入场 | TrendPullback |
| **R+V** | 震荡 | 高波动 | 假突破多，来回扫 | **STANDBY** |
| **R+L** | 震荡 | 低波动 | 区间稳定，均值回归有效 | BollMR |
...
| **仓位** | Regime-aware 缩放（confidence → scale） |
