//+------------------------------------------------------------------+
//|             XAUUSD_M5_Grid_Martingale_Pro_v3.1.mq5               |
//|             Copyright 2026, surrayi                              |
//|             M5黄金双向网格马丁 + 新闻 + 仪表盘 + 全风控 v3.1        |
//+------------------------------------------------------------------+
#property copyright "XAUUSD_M5_Grid_Martingale_Pro_v3.1.mq5 Copyright 2026, surrayi"
...
{   
    ...
    LogPrint(LOG_BASE, "=== XAUUSD M5 Grid Pro v3.2 已启动 ===");
...
void OnTick()
{
    ...
    if(AccountInfoDouble(ACCOUNT_EQUITY) < MinAccountEquity)
    {
        CloseAll("权益保护");
        return;
    }
   ...
   for(int i = 0; i < total; i++)
   {
      ...
      if(CalendarEventById(values[i].event_id, ev))
      {
         if(ev.importance >= CALENDAR_IMPORTANCE_HIGH &&
            MathAbs(values[i].time - TimeCurrent()) <= (NewsMinutesBefore + NewsMinutesAfter)*60) // 临近新闻
         {
...
{   
    ...
    if(basketCount == 0)
    {
        if(AutoStart) 
        {
            needOpen = true;
            if(IsNewBar())
                LogPrint(LOG_NORMAL, "✅ 满足首单条件(AutoStart=true)");
        }
        else
        {   
            if(IsNewBar())
                LogPrint(LOG_NORMAL, "❌ AutoStart=false, 无法开首单");
        }
    }
    else // 加仓逻辑
    {    
